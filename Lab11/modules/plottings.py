...
def plot_bayes_error_plots(llrs_list: list[ndarray], labels: ndarray, names: list[str], title: str, range=None,
                           llrs_precal_list: list[ndarray] = None) -> None:
    if len(llrs_list) != len(names):
        raise ValueError("llrs_list and names should have the same length, instead have "
                         "llrs_list: {len(llrs_list)} and names: {len(names)}. Then names are: {names}")
    if len(llrs_list) > 7:
        raise ValueError("Too many classifiers to plot. We only have 7 colors as of now")
    ...
    if llrs_precal_list is not None and len(llrs_precal_list) != len(llrs_list):
        raise ValueError("llrs_precal_list should have the same length as llrs_list")

    use_precal = False
    if llrs_precal_list is None:
        llrs_precal_list = llrs_list
    else:
        use_precal = True
    ...
    for (llrs, llrs_precal, name, color) in zip(llrs_list, llrs_precal_list, names, colors[:len(llrs_list)]):
        normalized_dcfs = []
        min_dcfs = []
        precal_dcfs = []

        for p in tqdm(effPriorLogOdds, desc=f"Computing DCFs for {name}..."):
            pi = 1 / (1 + np.exp(-p))
            dcf, _, _ = get_dcf__includes_classification(llrs, labels, pi, 1, 1)
            normalized_dcf = get_normalized_dcf_binary(pi, 1, 1, dcf)
            min_dcf = get_min_normalized_dcf_binary(llrs, labels, pi, 1, 1)

            if use_precal:  # precal = pre-calibrated
                dcf_precal, _, _ = get_dcf__includes_classification(llrs_precal, labels, pi, 1, 1)
                precal_normalized_dcf = get_normalized_dcf_binary(pi, 1, 1, dcf_precal)
                precal_dcfs.append(precal_normalized_dcf)

            normalized_dcfs.append(normalized_dcf)
            min_dcfs.append(min_dcf)

        if use_precal:
            plt.plot(effPriorLogOdds, min_dcfs, label=f"minDCF {name}", ls='dashed', color=color)
            plt.plot(effPriorLogOdds, precal_dcfs, label=f"actDCF (pre cal.) {name}", ls='dotted', color=color)
            plt.plot(effPriorLogOdds, normalized_dcfs, label=f"actDCF (cal.) {name}", color=color)
        else:
            plt.plot(effPriorLogOdds, normalized_dcfs, label=f"{name}", color=color)
            plt.plot(effPriorLogOdds, min_dcfs, label=f"min-{name}", ls='dashed', color=color)
    ...
    plt.savefig(f'output_first/bayes_error_plot_{title}.png')
    print(f"[[Saved bayes_error_plot_{title}.png]]")
    plt.clf()
