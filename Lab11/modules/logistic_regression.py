...
from modules.evaluation import get_dcf__includes_classification, get_min_normalized_dcf_binary, \
    get_normalized_dcf_binary, get_min_and_act_DCFs
...
class LogisticRegressionParams:
    ...
    def get_pi(self) -> float:
        """ Returns the empirical prior if the model is non-weighted, otherwise the prior used when training the weighted model"""
        return self.pi

    def get_variant(self) -> str:
        """ Returns the variant of the model, i.e. either 'nonweighted' or 'priorweighted' """
        return self.variant
...
def binary_logreg_train(DTR: np.ndarray, LTR: np.ndarray, l: float) -> Tuple[np.ndarray, float, float]:
    ...
    b = x[-1] - np.log(pi_emp / (1 - pi_emp))

    return w, b, pi_emp


def binary_logreg_classify(w, b, DVAL: np.ndarray, pi_emp) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    if DVAL.ndim != 2:
        raise ValueError(f"DVAL matrix should be nxm, instead is {DVAL.shape}")


    # Compute the log-posteriors and the predicted labels
    llrs = (vcol(w).T @ DVAL + b).ravel()
...
def binary_logreg_trainNeval(DTR: np.ndarray, LTR: np.ndarray, DVAL: np.ndarray, LVAL: np.ndarray,
                l: float, eff_pi,) -> Tuple[Tuple[np.ndarray, np.ndarray], Tuple[float, float]]:
    """ Train and evaluate a binary logistic regression model with non-weighted. """
    w, b, pi_emp = binary_logreg_train(DTR, LTR, l)
    llrs, logposteriors, _ = binary_logreg_classify(w, b, DVAL, pi_emp)
    dcf_min, dcf_norm = get_min_and_act_DCFs(llrs, LVAL, eff_pi)

    return (logposteriors, llrs), (dcf_norm, dcf_min)


def binary_logreg_prior_weighted_train(DTR: np.ndarray, LTR: np.ndarray, l: float, pi_weight) -> Tuple[np.ndarray, float, float]:
    """  Train a binary logistic regression model with non-weighted. """
    if DTR.ndim != 2:
        raise ValueError(f"DTR matrix should be nxm, instead is {DTR.shape}")
    if LTR.ndim != 1:
    ...
    # Optimize the objective function
    x0 = np.zeros(DTR.shape[0] + 1)
    x, f, d = scipy.optimize.fmin_l_bfgs_b(func=logreg_obj_prior_weighted, x0=x0, args=(DTR, LTR, l, pi_weight), approx_grad=False)
    w = x[:-1]  # Where: score = w.T @ x + b
    b = x[-1] - np.log(pi_weight / (1 - pi_weight))

    return w, b, pi_weight


def binary_logreg_prior_weighted_classify(w, b, DVAL: np.ndarray, pi_weight) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    if DVAL.ndim != 2:
        raise ValueError(f"DVAL matrix should be nxm, instead is {DVAL.shape}")
    ...
    logposteriors = llrs + np.log(pi_weight / (1 - pi_weight))
    predicted_labels = np.sign(logposteriors)
    predicted_labels[predicted_labels == -1] = 0
