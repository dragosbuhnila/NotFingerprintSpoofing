...
def get_min_normalized_dcf_binary(llrs, labels, pi, Cfn, Cfp, verbose=False, verbose_title=""):
    thresholds = sorted(llrs.ravel().tolist())
...
def get_dcf__includes_classification(llrs, labels, pi, Cfn, Cfp):
    # Are we sure that the threshold is not the empirical or weight one?
...
def get_min_and_act_DCFs(llrs: ndarray, L_val: ndarray, pi_eff: float) -> Tuple[float, float]:
    triplet = (pi_eff, 1, 1)

    dcf, _, _ = get_dcf__includes_classification(llrs, L_val, *triplet)
    dcf_norm = get_normalized_dcf_binary(*triplet, dcf=dcf)  # This is the actual DCF normalized
    dcf_min = get_min_normalized_dcf_binary(llrs, L_val, *triplet)

    return dcf_min, dcf_norm

def get_act_DCF(llrs: ndarray, L_val: ndarray, pi_eff: float) -> float:
    triplet = (pi_eff, 1, 1)
