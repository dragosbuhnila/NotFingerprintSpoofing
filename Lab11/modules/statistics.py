...
def get_correlation_matrix(C):
    correlation_matrix = C / (vcol(C.diagonal() ** 0.5) * vrow(C.diagonal() ** 0.5))
...
def get_empirical_prior_binary(labels):
    unique_classes = get_unique_classes(labels)
    if len(unique_classes) != 2:
        raise ValueError("This function is defined only for binary classification tasks")

    nof_samples = len(labels)
    nof_positive_samples = how_many_foreach_class(labels)[unique_classes[1]]

    return nof_positive_samples / nof_samples
