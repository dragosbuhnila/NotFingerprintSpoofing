import scipy
from modules.classification import classify_with_threshold, get_classification_err, get_dumb_threshold, get_prior_cost_threshold
...
def extract_densities_of_MVG_or_variants(logscore, priors, logSJointCheck=None, logMarginalCheck=None, logPosteriorCheck=None, verbose=False):
    ...
    # ==================================================
    # Compute marginal
    logscore_marginal = vrow(scipy.special.logsumexp(logscore_joint, axis=0))
...
def classify_nclasses(logscore_posterior, L_val, verbose=False, verbose_only_err=False):
    score_posterior = np.exp(logscore_posterior)

    # Compute best classes sample by sample
    predictions = np.argmax(score_posterior, axis=0)        ### I THINK IT'S BETTER TO MODIFY PREDICTIONS BUT I'LL LEAVE IT FOR THE FUTURE
    ...
    return nof_errors, accuracy, error_rate, predictions


""" Priors are set to 50%. Same problems as for classify_nclasses() may hold. """
def classify_two_classes(logscore, L_val, verbose=False, verbose_only_err=False, prior_one=0.5, prior_two=0.5):
    if (len(get_unique_classes(L_val)) != 2):
        raise ValueError(f"Tried to perform binary classificatio with {len(get_unique_classes(L_val))} classes")

    llrs = get_llrs(logscore)
...
def opt_classify_two_classes(llrs, L_val, threshold, verbose=False, verbose_only_err=False):
    if (len(get_unique_classes(L_val)) != 2):
